...
let rec parseE stream =
  ...
  *)
  ...
  (match (peekAtFirstToken stream) with
    ...
    | ((BodyToken _) | (FunctionToken _)) ->
      (print_endline "E -> fun ident -> E");
      (inject stream >>=
      parseE >>=
      accept BodyToken >>=
    ...
    | ((LetToken) | (IdentToken _) | EqualToken | InToken) ->
    ...
    (* regle #5 - E -> ER EX*)
    | ( (NumberToken _) | TrueToken | FalseToken | MinusToken | LeftParenthesisToken ) ->
        (print_endline "ER EX");
        (inject stream >>=
        parseER >>=
        parseEX)
...
and parseARG stream =
  ...
  (match (peekAtFirstToken stream) with
    ...
    (* regle 26 *)
    | (TimesToken | DivideToken | PlusToken | MinusToken | EqualToken  | EOSToken)  ->
        (print_endline "");
        (Success stream)
    | _ -> Failure)
;;
